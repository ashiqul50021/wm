...
<section class="content-main">
    <div class="content-header">
        <h3 class="content-title">Account Ledgers <span class="badge rounded-pill alert-success"> {{$account_ledgers->count() }} </span></h3>
        <div>
            <a href="{{ route('vendor.accounts.ledgers.create') }}" class="btn btn-primary"><i class="material-icons md-plus"></i>Add New</a>
        </div>
    </div>
    </div>
    <div class="card mb-4">
        <div class="card-body">
            <form action="{{ route('vendor.accounts.leagers.report') }}" method="post">
                @csrf
                            ...
                            <td class="text-end">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        {{-- @if(Auth::guard('vendor')->user()->role == '2' || in_array('52', json_decode(Auth::guard('admin')->user()->staff->role->permissions))) --}}
                                        @if(Auth::guard('vendor')->user()->role == '2')
                                            <a class="dropdown-item text-danger" href="{{ route('vendor.accounts.ledgers.delete',$ledger->id) }}" id="delete">Delete</a>
                                        @endif
                                    </div>
                                </div>
                                <!-- dropdown //end -->
