...
<body>
    ...
    <script type="text/javascript">
        ...
        function productView(id){
            var url = "{{route('ProductViewAjax', '')}}"+"/"+id;
                ...
                success:function(data){
                    ...
                    if(data.product.discount_price>0){
                        if(data.product.discount_type==1){
                            ...
                            $('#pprice').text((data.product.regular_price - discount).toFixed(2));
                        ...
                        $('#pprice').text((data.product.regular_price).toFixed(2));
                        $('#oldprice').text('');
                    }
                    ...
                    $.each(data.attributes, function(key,value){
                        i++;
                        html +='<div class="attr-detail attr-size">';
                        ...
                        // });
                        html += '<p>'+data.product.variant_name+'</>'
        ...
        function productVariantView(id, vid)
        {
            var product_id = id;
            var variant_id = vid;
            id = id + '_' + vid;
            var url = "{{route('ProductViewModal', '')}}"+"/"+id;
            $.ajax({
                type:'GET',
                url: url,
                dataType:'json',
                success:function(data){
                    const {id,product_code,name_en,category,brand,product_thumbnail,stock_qty,regular_price,minimum_buy_qty} = data.product;
                  console.log(data)
                  console.log(stock_qty)
                  $('#product_name').text(name_en);
                    $('#pname').val(name_en);
                    $('#product_id').val(id);
                    $('#pcode').text(product_code);
                    $('#pcategory').text(category.name_en);
                    $('#pbrand').text(brand.name_en);
                    $('#pimage').attr('src', '/' + product_thumbnail);
                    $('#stock_qty').val(stock_qty);
                    $('#minimum_buy_qty').val(minimum_buy_qty);
                    $('#pprice').text(regular_price);
                    $('#product_price').val(regular_price);
        ...
        function addToCart(){
            ...
            var url = "{{route('cart.add', '')}}"+"/"+id;
            $.ajax({
            type:'POST',
            url:url,
            dataType:'json',
            data:{
              color:color,size:size,quantity:quantity,product_name:product_name,product_price:price,options:jsonString,
            },
                success:function(data){
                    // console.log(data);
                    miniCart();
                                ...
                                <td class="text-center detail-info" data-title="Stock">
                                    <div class="detail-extralink mr-15">
                                        ...
                                        ${value.qty > 1
                                          ...
                                          ? `<button type="submit" style="margin-right: 5px; background-color: #cb1e25 !important; font-size: 12px;" class="btn btn-sm" id="${value.rowId}" onclick="cartDecrement(this.id)" >-</button>`
