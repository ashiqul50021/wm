...
@section('content-frontend')
            ...
            <div class="carausel-10-columns-cover position-relative">
                <div class="carausel-10-columns" id="carausel-10-columns">
                    @foreach ($featured_category as $cat)
                        <div class="card-2 bg-9 d-flex flex-column justify-content-center align-items-center wow animate__animated animate__fadeInUp najmul___category"
                            data-wow-delay=".1s">
                                ...
                                <a href="{{ route('product.category', $cat->slug) }}">
                                    @if ($cat->image && $cat->image != '' && $cat->image != 'Null')
                                        <img class="default-img lazyload img-responsive"
                                            ...
                                            src="{{ asset($cat->image ?? 'upload/product-default-sm.jpg') }}" alt="">
                                    @else
                ...
                <ul class="nav nav-tabs links" id="myTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        ...
                        <button class="nav-link active" id="nav-tab-one" data-bs-toggle="tab" data-bs-target="#tab-one"
                            ...
                            data-category-slug="all">All</button>
                    ...
                    @foreach (get_categories()->take(5) as $category)
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="nav-tab-two" data-bs-toggle="tab"
                                ...
                                data-category-slug="{{ $category->slug }}">{{ $category->name_en }}</button>
                        </li>
                    @endforeach
    ...
    <div class="button-load">
        <button class="btn btn-primary d-flex mx-auto" id="load-more-button">Load more</button>
    </div>
    <!--Products Tabs-->
    ...
    @php
        $sellers = App\Models\Seller::where('status',1)->latest()->get();
    @endphp
                ...
                <div class="carausel-10-columns-cover position-relative">
                    <div class="carausel-10-columns-vendors" id="carausel-10-columns-vendors">
                        @foreach ($sellers as $seller)
                            <div class="card-2 mx-2 bg-9 d-flex flex-column justify-content-center align-items-center wow animate__animated animate__fadeInUp najmul___category"
                                data-wow-delay=".1s">
                                <figure class="img-hover-scale overflow-hidden">
                                    <a href="{{ route('seller.product', $seller->slug) }}">
                                        @if ($seller->shop_profile && $seller->shop_profile != '' && $seller->shop_profile != 'Null')
                                            <img class="default-img" src="{{ asset($seller->shop_profile) }}"
                                                alt="" />
                                        @else
                        ...
                        @foreach ($product_top_sellings as $product_top_selling)
                            <article class="row align-items-center hover-up">
                                ...
                                <div class="col-md-8 mb-0">
                                    ...
                                    {{-- start new --}}
                                    <div class="product-rate-cover">
                                        @if ( $product_top_selling->productReview)
                                            @php
                                                $productReviewAverage = \App\Models\ProductReview::where('status', 1)
                                                    ->where('product_id',  $product_top_selling->id)
                        ...
                        @foreach ($product_trendings as $product_trending)
                            <article class="row align-items-center hover-up">
                                ...
                                <div class="col-md-8 mb-0">
                                    ...
                                    {{-- start new  --}}
                                    <div class="product-rate-cover">
                                        @if ($product_trending->productReview)
                                            @php
                                                $productReviewAverage = \App\Models\ProductReview::where('status', 1)
                                                    ->where('product_id', $product_trending->id)
                                                    ->average('start_rating');
                                                $productReviewCount = \App\Models\ProductReview::where('status', 1)
                                                    ->where('product_id',  $product_trending->id)
                                                    ->count();
                        ...
                        {{-- @dd($product_recently_add->id); --}}
                            <article class="row align-items-center hover-up">
                                ...
                                <div class="col-md-8 mb-0">
                                    ...
                                    {{-- start new  --}}
                                    <div class="product-rate-cover">
                                        @if ($product_recently_add->productReview)
                                            @php
                                                $productReviewAverage = \App\Models\ProductReview::where('status', 1)
                                                    ->where('product_id', $product_recently_add->id)
                                                    ->average('start_rating');
                                                $productReviewCount = \App\Models\ProductReview::where('status', 1)
                                                    ->where('product_id', $product_recently_add->id)
                                                    ->count();
                                            ...
                                            {{-- @dd($productReviewAverage); --}}
                                            {{-- @dd($productReviewCount); --}}
                                            @if ($productReviewAverage > 0 && $productReviewAverage < 2)
                                                <div class="d-flex">
                                                    <p style="color: orange"><i class="fas fa-star"></i></p>
                                                    <p><i class="fas fa-star"></i></p>
                        ...
                        @foreach ($product_top_rates as $product_top_rate)
                            <article class="row align-items-center hover-up">
                                ...
                                <div class="col-md-8 mb-0">
                                    ...
                                    {{-- start new  --}}
                                    <div class="product-rate-cover">
                                        @if ($product_top_rate->productReview)
                                            @php
                                                $productReviewAverage = \App\Models\ProductReview::where('status', 1)
                                                    ->where('product_id', $product_top_rate->id)
                                                    ->average('start_rating');
                                                $productReviewCount = \App\Models\ProductReview::where('status', 1)
                                                    ->where('product_id', $product_top_rate->id)
                                                    ->count();
...
<script>
    $(document).ready(function() {
        ...
        loadMoreButton.click(function() {
            ...
            var categorySlug = activeTab.data('category-slug');
            ...
            if (categorySlug === 'all') {
                newUrl = '{{ route('product.show') }}';
            } else {
                newUrl = '{{ route('product.category', ['slug' => ':slug']) }}'.replace(':slug',
                    categorySlug);
            }
