...
@section('admin')
    ...
    <section class="content-main">
        <div class="content-header">
            <div>
                <h2 class="content-title card-title">All Order Sale Report</h2>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <!-- card-header end// -->
                    <div class="card-body">
                        <form class="" action="{{ route('searchByAllOrderSaleReport') }}" method="GET">
                            <div class="form-group row mb-3 ">
                                <div class="col-md-6">
                                    <label class="col-form-label"><span>All Order Sale Orders :</span></label>
                                </div>
                            ...
                            <div class="table-responsive-sm">
                                <table id='example' class="table table-bordered table-hover" width="100%">
                                            ...
                                            @foreach ($order as $key => $order)
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        @elseif($order->due_amount > 0 || $order->due_amount < $order->grand_total)
                                    ...
                                    <tr>
                                        ...
                                        <td>Discount = {{ number_format($order->sum('discount') ?? 0, 2) }}</td>
                                        <td>Due = {{ number_format($order->sum('due_amount') ?? 0, 2) }}</td>
                                        <td></td>
                                        <td>Total Paid = {{ number_format($order->sum('paid_amount') ?? 0, 2) }}</td>
                                        <td>Grand Total = {{ number_format($order->sum('grand_total') ?? 0, 2) }}</td>
                                    </tr>
                                </table>
