...
@section('admin')
    ...
    <section class="content-main">
        ...
        <form action="{{ route('order.dealer.dealerReorderStore') }}" method="post" >
            @csrf
            <div class="card">
                <!-- card-header end// -->
                <div class="card-body">
                        ...
                        <div class="col-md-12 mt-40">
                            <table class="table table-bordered">
                                <tbody>
                                    ...
                                    <tr>
                                        <th>Total Amount</th>
                                        <td id="totalAmountTop">{{ $dealer->total_amount }} <strong>Tk</strong></td>
                                    </tr>
                        ...
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                        {{-- @dd($dealer->dealerDue) --}}
                                        @foreach ($dealer->dealerDue as $key => $dealer_request_product)
                                            <tr>
                                                <td>
                                                    @if ($dealer_request_product->product->stock_qty < $dealer_request_product->qty)
                                                        <input class="form-check-input checkedItem" type="checkbox"
                                                            value="{{ $dealer_request_product->product->id }}"
                                                ...
                                                {{-- <td class="requestQuantity" data-id="{{ $dealer_request_product->id }}">
                                                    ...
                                                    <input type="hidden"
                                                        name="requested_qty[{{ $dealer_request_product->product->id }}]"
                                                        value="{{ $dealer_request_product->qty ?? '0' }}">
                                                </td> --}}
                                                <td>
                                                    <input type="number"
                                                        name="confirm_qty[]"
                                                        min="1" class="form-control quantity" readonly
                                                        data-id="{{ $dealer_request_product->id }}"
                                                        value="{{ $dealer_request_product->qty ?? '0' }}">
                                                </td>
                                                ...
                                                {{-- <td>
                                                    <input type="hidden"
                                                        name="due_quantity[{{ $dealer_request_product->product->id }}]"
                                                        data-id="{{ $dealer_request_product->id }}"
                                                ...
                                                <td class="productTotalPrice" data-id="{{ $dealer_request_product->id }}">
                                                    ...
                                                    {{ $dealer_request_product->product->wholesell_price * $dealer_request_product->qty ?? '0.00' }}
                                                </td>
                                                <input type="hidden"
                                                    name="product_total_price[]"
                                                    class="productTotalPriceInput"
                                                    data-id="{{ $dealer_request_product->id }}"
                                                    value="{{ $dealer_request_product->product->wholesell_price * $dealer_request_product->qty ?? '0.00' }}">
                                            </tr>
                            ...
                            <!-- table-responsive// -->
                            @if ($dealer->dealerDue->isNotEmpty())
...
@push('footer-script')
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#totalAmountTop").text(totoalPricevalue.toFixed());
